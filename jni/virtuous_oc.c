...
{
    chmod(SYS_CGOV_C0, S_IRWXU|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH); // chmod 755 to allow us to write to these values
    chmod(SYS_CMAX_C0, S_IRWXU|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);
    chmod(SYS_CMIN_C0, S_IRWXU|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);
    ...
    if (write_to_file(SYS_CMIN_C0, min_freq) != 0)
        return -1;
    chmod(SYS_CGOV_C0, S_IRUSR|S_IXUSR|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH); // chmod 555 to prootect these values from other apps overwriting
    chmod(SYS_CMAX_C0, S_IRUSR|S_IXUSR|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);
    chmod(SYS_CMIN_C0, S_IRUSR|S_IXUSR|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);  
    write_to_file(SYS_CGOV_C1, governor); // not really necessary as newer S4 dual cores devices share values for these in CPU0 these values do not exist on these devices
